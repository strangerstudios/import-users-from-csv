	...
	if($status.length > 0)
	{
		...
		function ai_importPartial()
		{
			jQuery.ajax({
				...
				dataType: 'html',
				data: 'action=import_users_from_csv&filename='+ai_filename+'&password_nag='+ai_password_nag+'&users_update='+ai_users_update+'&new_user_notification='+ai_new_user_notification,
				error: function(xml){
